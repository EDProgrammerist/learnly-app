...
// CHANGE 1: Import the Google provider instead of OpenAI
...
Only return an empty array [] if there are truly NO partners with any related learning interests.`;
    ...
    const { text } = await generateText({
      // CHANGE 2: Use Google Gemini Flash model (Free Tier)
    ...
    try {
      ...
      if (arrayMatch) {
        ...
        // Don't throw here, just return empty matches to prevent crash
        return {
             matched: 0,
             matches: [],
...
Please format your response as JSON with this structure:
{
  ...
  try {
    const { text } = await generateText({
      // CHANGE 3: Use Google Gemini Flash model here too
      model: google("gemini-2.5-flash"), 
      prompt,
    });
